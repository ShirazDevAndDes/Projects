...
if($_SERVER["REQUEST_METHOD"] == "POST"){
    ...
    if($email_validate){
        ...
        $email_check_sql = "SELECT * FROM student WHERE student_email='$email'";
        ...
        if($email_check_result->num_rows > 0){
            ...
            $user_id = $email_check_row['student_id'];
            $user_email = $email_check_row['student_email'];

            $reset_token = md5($user_email).rand(2, 6);

            $email_check_sql = "UPDATE student SET student_reset_token='$reset_token' WHERE student_id='$user_id'";
            $email_check_result = $conn->query($email_check_sql);
            ...
            if($email_check_result){
                ...
                $reset_link = (isset($_SERVER['HTTPS']) && $_SERVER['HTTPS'] === 'on' ? "https" : "http") . "://". $_SERVER['HTTP_HOST'] ."/reset_password.php?user=student&user_email=". $user_email ."&token=". $reset_token;
